...
def create_data_structure(string_input):
    ...
    #print list
    for element in list:
        gamer = element[:element.find(' ')]
        #print network
                
        if element == '':
            break
        elif gamer not in network:
            sub = {}
            sub["Games"] = []
            sub["Friends"] = []
            network[gamer] = sub
        ...
        if ' likes to play ' in element:
            games = element.split(' likes to play ')[1]
            network[gamer]['Games'] = games.split(', ')
        else:
            #print element
            connections = element.split(' is connected to ')[1]
            network[gamer]['Friends'] = connections.split(', ')  
    return network
...
def add_connection(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        return False
    elif user_B not in network[user_A]['Friends']:
...
def add_new_user(network, user, games):
    if user not in network:
        ...
        new_user['Friends']=[]
        network[user]=new_user
    else:
        for entry in games:
            if entry not in network[user]['Games']:
                network[user]['Games'].append(entry)
    return network
...
def get_secondary_connections(network, user):
    if user not in network:
        ...
        first_connections=network[user]['Friends']
        secondary_connections=[]
        for connection in first_connections:
            if connection in network:
                secondary_connections+=network[connection]['Friends']
...
def connections_in_common(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        ...
        for element in network[user_A]['Friends']:
            if element in network[user_B]['Friends']:
                count+=1
    return count
    ...
    if user_A not in network or user_B not in network:
        ...
        for node in network[user_A]['Friends']:
            if node not in visited_node:
                path+=[user_A]+path_to_friend(network, node, user_B)
                if path:
                    return path
    return None
...
#   remove user_B from user_A's connections. 
# Arguments:
#   network: The network you created with create_data_structure. 
#   user_A:  String holding the starting username ("Abe")
#   user_B:  String holding the ending username ("Zed")
# Return: 
#   A list of updated connections for user_A.
#   - If user_A or user_B is not in network, return False.
#   - If the user has no connections, return an empty list.
...
def remove_connection(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        return False
    connections=network[user_A]['Friends']
...
#print get_connections(net, "Alice")#print get_games_liked(net, "John")
#print get_games_liked(net, "Bob")
#print add_connection(net, "John", "Walter")
...
#print connections_in_common(net, "Debra", "Freda")
#print path_to_friend(net, "John", "John")
#print path_to_friend(net, "John", "Olive")
#print path_to_friend(net, "Alice", "Bob")
#print remove_connection(net, 'John', 'Walter')
#print remove_connection(net, 'Nick', 'Walter')
